...
void main() {
  ...
  // Distance/ETA now live in `title` (Android renders it bold/prominent —
  // the one number a half-asleep rider needs at a glance); destination and
  // Monitoring status live in `body`, the lighter secondary line.
  group('redundant updates are suppressed', () {
    test('a stationary vehicle produces identical text', () {
      ...
      expect(text(842).title, text(842).title);
    });

    test('sub-metre GPS jitter does not change the text', () {
      expect(text(842.2).title, text(841.8).title);
    });
    ...
    test('drift within the same 10 m bucket does not change the text', () {
      ...
      expect(text(841).title, text(844).title); // both -> 840 m
      expect(text(846).title, text(849).title); // both -> 850 m
    });

    test('crossing a 10 m boundary DOES change the text', () {
      expect(text(842).title, isNot(text(858).title));
    });
  });

  group('formatting', () {
    test('rounds sub-kilometre distance to 10 m', () {
      expect(text(846).title, contains('850 m away'));
      expect(text(844).title, contains('840 m away'));
    });

    test('switches to kilometres at 1 km with one decimal', () {
      expect(text(1000).title, contains('1.0 km away'));
      expect(text(7240).title, contains('7.2 km away'));
    });

    test('includes the destination and monitoring state', () {
      final t = text(500);
      expect(t.body, contains('Approaching PUP Sta. Mesa'));
      expect(t.body, contains('Monitoring'));
    });

    test('ETA is shown in whole minutes, and omitted when unknown', () {
      expect(text(500, eta: 8.4).title, contains('ETA 8 min'));
      expect(text(500).title, isNot(contains('ETA')));
    });
  });
}
